    ...
    def install_steamcmd(self, force=False):
        ...
        # Installs steamcmd for Linux
            ...
            if os.path.exists(os.path.join(self.steamcmd_path, 'steamcmd.sh')):
                result['error'] = True
                result['message'] = "Steamcmd already installed. No need for a reinstall."
            else:
                ...
                with tarfile.open(os.path.join(self.steamcmd_path, 'steamcmd_linux.tar.gz'), 'r:gz') as z:
                    ...
                    safe_extract(z, self.steamcmd_path)
                result['error'] = False
                result['message'] = "Steamcmd installed to ./{}".format(self.steamcmd_path)
        else:
            result['error'] = True
            result['message'] = "Unsupported system. I detected {}.".format(self.platform)
